...
namespace LibHac.NcaLegacy
{
    public class Nca : IDisposable
    {
        ...
        {
	        var orderedSections = Sections.Where(x => x != null).OrderBy(x => x.Offset).ToArray();
			var list = new List<IStorage> { OpenHeaderStorage((int)orderedSections[0].Offset) };
	        list.AddRange(orderedSections.Select(section => OpenRawSection(section.SectionNum, true)));
        ...
        private CachedStorage OpenHeaderStorage(int size = 0)
        {
	        if (size == 0)
	        {
